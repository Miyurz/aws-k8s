...
function spin_k8s_cluster() {
 ...
 aws-vault exec admin --no-session --assume-role-ttl=1.5h -- env \
  ...
  #AWS resources
  --admin-access=0.0.0.0/0       `# Restrict API access to this CIDR.  If not set, access will not be restricted by IP. (default [0.0.0.0/0])`\
  ...
  --authorization RBAC           `# Authorization mode to use: AlwaysAllow or RBAC (default "RBAC")`\
  --channel stable               `# Channel for default versions and configuration to use (default "stable"), More here: https://github.com/kubernetes/kops/tree/master/channels`\
